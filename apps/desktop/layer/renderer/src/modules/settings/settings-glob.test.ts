...
describe("settings route metadata", () => {
  it("uses route handles while preserving the modal metadata mapping", () => {
    ...
    expect(pages.openai?.loader).toBeDefined()
    // The local edition adds a page for the embedded service (background refresh, backup).
    expect(pages.local?.loader).toMatchObject({ name: "titles.local" })
